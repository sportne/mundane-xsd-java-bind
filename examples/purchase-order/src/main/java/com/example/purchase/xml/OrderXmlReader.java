...
/** Generated XML reader for {@link com.example.purchase.Order}. */
public final class OrderXmlReader {
  ...
  public static com.example.purchase.Order read(
    ...
    if (input.kind() != io.github.mundanej.mxjb.runtime.XmlEventKind.START_ELEMENT
      ...
      throw readException(input, "MXJB-GR-001", "Expected root element {urn:purchase}order.");
    }
    com.example.purchase.Order value = readOrder(input, NAME_1);
  ...
  private static com.example.purchase.Order readOrder(
      io.github.mundanej.mxjb.runtime.XmlEventReader input,
    ...
    java.util.ArrayList<com.example.purchase.Line> lineValues = new java.util.ArrayList<>();
    ...
    while (true) {
      ...
      if (NAME_3.equals(input.name())) {
        if (1 < lastElementOrder) {
          throw readException(input, "MXJB-GR-002", "Out-of-order XML element {urn:purchase}id.");
        }
        lastElementOrder = Math.max(lastElementOrder, 1);
        if (id != null) {
          throw readException(input, "MXJB-GR-005", "Repeated XML element {urn:purchase}id.");
        }
        id = readStringElement(input, NAME_3);
      } else if (NAME_4.equals(input.name())) {
        if (2 < lastElementOrder) {
          throw readException(input, "MXJB-GR-002", "Out-of-order XML element {urn:purchase}note.");
        }
        lastElementOrder = Math.max(lastElementOrder, 2);
        if (note.isPresent()) {
          throw readException(input, "MXJB-GR-005", "Repeated XML element {urn:purchase}note.");
        }
        note = java.util.Optional.of(readStringElement(input, NAME_4));
      } else if (NAME_5.equals(input.name())) {
        if (3 < lastElementOrder) {
          throw readException(input, "MXJB-GR-002", "Out-of-order XML element {urn:purchase}line.");
        }
    ...
    if (id == null) {
      throw readException(input, "MXJB-GR-004", "Missing required XML element {urn:purchase}id.");
    }
    if (lineValues.isEmpty()) {
      throw readException(input, "MXJB-GR-004", "Missing required XML element {urn:purchase}line.");
    }
    input.next();
    return new com.example.purchase.Order(version, id, note, java.util.List.copyOf(lineValues));
  }

  private static com.example.purchase.Line readLine(
      io.github.mundanej.mxjb.runtime.XmlEventReader input,
      io.github.mundanej.mxjb.runtime.XmlName elementName)
      throws io.github.mundanej.mxjb.runtime.XmlReadException {
    expectStart(input, elementName);
    ...
    while (true) {
      ...
      if (NAME_6.equals(input.name())) {
        if (1 < lastElementOrder) {
          throw readException(input, "MXJB-GR-002", "Out-of-order XML element {urn:purchase}sku.");
        }
        lastElementOrder = Math.max(lastElementOrder, 1);
        if (sku != null) {
          throw readException(input, "MXJB-GR-005", "Repeated XML element {urn:purchase}sku.");
        }
        sku = readStringElement(input, NAME_6);
      } else if (NAME_7.equals(input.name())) {
        if (2 < lastElementOrder) {
          throw readException(
              input, "MXJB-GR-002", "Out-of-order XML element {urn:purchase}quantity.");
        }
        lastElementOrder = Math.max(lastElementOrder, 2);
        if (quantity != null) {
          throw readException(input, "MXJB-GR-005", "Repeated XML element {urn:purchase}quantity.");
        }
        quantity = readIntElement(input, NAME_7);
      } else {
        throw readException(input, "MXJB-GR-002", "Unexpected XML element.");
      }
    }
    ...
    if (sku == null) {
      throw readException(input, "MXJB-GR-004", "Missing required XML element {urn:purchase}sku.");
    }
    if (quantity == null) {
      throw readException(
          input, "MXJB-GR-004", "Missing required XML element {urn:purchase}quantity.");
    }
    input.next();
    return new com.example.purchase.Line(sku, quantity);
  }
