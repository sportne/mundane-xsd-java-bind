...
Specification references: `docs/architecture/runtime-architecture.md`, `docs/architecture/module-boundaries.md`, `docs/architecture/native-image-architecture.md`, `docs/verification/native-image-test-plan.md`
...
- `runtime-core` primitives and pull-style XML interfaces exist under `io.github.mundanej.mxjb.runtime`.
- Runtime tests and ArchUnit checks cover value invariants, immutable validation results, checked exception diagnostic retention, generated-code-facing interface shape, and forbidden dynamic/runtime dependencies.
- Native smoke execution passes with SDKMAN GraalVM CE 21.0.2 when run with explicit `JAVA_HOME` and configuration cache disabled.
- Native test wiring disables the optional GraalVM reachability metadata repository for this dependency-free runtime smoke lane and filters JVM-only ArchUnit checks out of the native executable. JVM `check` still runs the architecture tests.
