...
Tests to add/update: unit tests for value semantics, null/argument invariants, diagnostic stability, exception data retention, no-third-party dependency enforcement, and ArchUnit module-boundary rules
Documentation to update: runtime architecture, module README, traceability, and conformance docs as needed
Commands to run: `./gradlew :modules:runtime-core:test :modules:runtime-core:check`, `./gradlew validateDesignControlPack qualityGate`, `git diff --check`
...
- Native Image: interfaces and value types must be Native Image friendly by default.
