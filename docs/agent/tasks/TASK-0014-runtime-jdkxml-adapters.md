...
Expected behavior: optional adapters from JDK XML APIs to `runtime-core` XML event/output interfaces for tests and examples, with secure defaults for entity/resource behavior and stable location/diagnostic propagation where available.
...
- Added `io.github.mundanej.mxjb.runtime.jdkxml.JdkXmlAdapters` as the public optional StAX adapter entry point.
- Added package-private StAX input/output adapters for `XmlEventReader` and `XmlOutput`.
- Added secure StAX factory defaults that disable DTD and external entity support and deny external resource resolution.
- Added positive reader/writer tests, negative external entity tests, diagnostic wrapping tests, and runtime-jdkxml architecture tests.
- Verified the adapter tests under SDKMAN GraalVM CE 21.0.2 with `:modules:runtime-jdkxml:nativeTest`.
- Kept generated-code and `runtime-core` dependency boundaries unchanged.
