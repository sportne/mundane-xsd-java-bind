...
Allowed files: generator-core tests/resources/golden fixtures, generator-core test harness code, narrow `modules/generator-core/build.gradle` generated-source smoke wiring, `modules/testing-support/src/test` or test-helper source if required by accepted design, and directly related verification/traceability docs
...
Commands to run: `./gradlew :modules:generator-core:test :modules:generator-core:check`, `./gradlew :modules:generator-core:generatedCodeNativeSmoke` with GraalVM/native-image available or document the concrete blocker, `./gradlew validateDesignControlPack qualityGate`, `git diff --check`
...
- `GeneratedSourceVerifier` provides reusable generated-source golden comparison, duplicate path detection, deterministic source comparison, Java 21 `-Xlint:all -Werror` compilation, and class loading for behavior tests.
- Approved golden source fixtures use `.java.golden` resources to prevent formatter rewrites while preserving generated relative paths.
- Model and writer emitter tests use the shared verifier for generated-source compilation and deterministic checks.
- `generatedCodeSmoke` compiles approved generated model/writer fixtures plus a test-only smoke main and runs it as part of `:modules:generator-core:check`.
- `generatedCodeNativeSmoke` builds and runs a Native Image executable containing only approved generated fixtures, `runtime-core`, and the smoke main; it passed with SDKMAN GraalVM CE 21.0.2 using explicit `JAVA_HOME` and `PATH`.
