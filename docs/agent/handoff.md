...
17. `TASK-0017` through `TASK-0021`: Draft backlog for the first public vertical slice. Not approved for implementation until each prior gate is accepted.
18. `TASK-0022` through `TASK-0046`: Draft post-0.1.0 vertical-slice backlog. Not approved for implementation until each slice planning task is accepted.

`TASK-0017` is the next draft task, but it is not approved for implementation until the round-trip examples and conformance-fixture scope is reviewed and explicitly promoted.
...
Implementation for `TASK-0017` remains blocked until it is promoted from draft to approved. The next readiness review should confirm:
...
- `TASK-0017` must add round-trip examples and conformance fixture expansion only; it must not add public generator API, CLI behavior, or Gradle plugin behavior.
