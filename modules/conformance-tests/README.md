...
- Opt-in `xs:choice`, `XP-VALIDATION-10-BASIC` facet, `XP-XSD10-COMPOSED` composed-schema,
  `XP-XSD10-SEMANTIC` semantic, and accepted `XP-XSD10-DOCUMENT` wildcard fixtures compared with
...
`xs:any`, mixed content, identity constraints, and XSD 1.1 are outside this harness until their task
cards are accepted.
