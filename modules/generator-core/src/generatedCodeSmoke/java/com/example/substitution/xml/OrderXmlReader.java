...
/** Generated XML reader for {@link com.example.substitution.Order}. */
public final class OrderXmlReader {
  ...
  public static com.example.substitution.Order read(
    ...
    if (input.kind() != io.github.mundanej.mxjb.runtime.XmlEventKind.START_ELEMENT
      ...
      throw readException(input, "MXJB-GR-001", "Expected root element {urn:substitution}order.");
    }
    com.example.substitution.Order value = readOrder(input, NAME_1);
  ...
  private static com.example.substitution.Order readOrder(
      io.github.mundanej.mxjb.runtime.XmlEventReader input,
    ...
    String id = null;
    java.util.Optional<com.example.substitution.PaymentSubstitution> payment =
    ...
    while (true) {
      ...
      if (NAME_2.equals(input.name())) {
        if (1 < lastElementOrder) {
          throw readException(
              input, "MXJB-GR-002", "Out-of-order XML element {urn:substitution}id.");
        }
        lastElementOrder = Math.max(lastElementOrder, 1);
        if (id != null) {
          throw readException(input, "MXJB-GR-005", "Repeated XML element {urn:substitution}id.");
        }
        id = readStringElement(input, NAME_2);
      } else if (NAME_3.equals(input.name())) {
        if (2 < lastElementOrder) {
          throw readException(
              input, "MXJB-GR-002", "Out-of-order XML element {urn:substitution}payment.");
        }
        lastElementOrder = Math.max(lastElementOrder, 2);
        if (payment.isPresent()) {
          throw readException(input, "MXJB-GR-005", "Repeated XML substitution payment.");
        }
        payment =
            java.util.Optional.of(
                new com.example.substitution.PaymentSubstitutionBranch(readPayment(input, NAME_3)));
      } else if (NAME_5.equals(input.name())) {
        if (2 < lastElementOrder) {
          throw readException(
              input, "MXJB-GR-002", "Out-of-order XML element {urn:substitution}cardPayment.");
        }
        lastElementOrder = Math.max(lastElementOrder, 2);
        if (payment.isPresent()) {
          throw readException(input, "MXJB-GR-005", "Repeated XML substitution payment.");
    ...
    if (id == null) {
      throw readException(
          input, "MXJB-GR-004", "Missing required XML element {urn:substitution}id.");
    }
    input.next();
    return new com.example.substitution.Order(id, payment);
  }

  private static com.example.substitution.Payment readPayment(
      io.github.mundanej.mxjb.runtime.XmlEventReader input,
      io.github.mundanej.mxjb.runtime.XmlName elementName)
      throws io.github.mundanej.mxjb.runtime.XmlReadException {
    expectStart(input, elementName);
    ...
    while (true) {
      ...
      if (NAME_4.equals(input.name())) {
        ...
        amount = readDecimalElement(input, NAME_4);
      } else {
        throw readException(input, "MXJB-GR-002", "Unexpected XML element.");
      }
    }
    ...
    input.next();
    return new com.example.substitution.Payment(amount);
  }
  ...
  private static com.example.substitution.Cardpayment readCardpayment(
    ...
    while (true) {
      ...
      if (NAME_4.equals(input.name())) {
        ...
        amount = readDecimalElement(input, NAME_4);
      } else if (NAME_6.equals(input.name())) {
        if (2 < lastElementOrder) {
          throw readException(
        ...
        cardlast4 = readStringElement(input, NAME_6);
      } else {
        throw readException(input, "MXJB-GR-002", "Unexpected XML element.");
      }
    }
